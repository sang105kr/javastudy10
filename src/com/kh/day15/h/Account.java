...
public class Account {
  ...
  //  기능 :  입금, 출금, 조회
  //입금
  public void deposit(int money) throws RuntimeException{
    //1회 입금한도는 4만원을 초과할 수 없다.
    if (money > 40_000) {
      throw new RuntimeException("1회 입금한도는 4만원을 초과할 수 없습니다.");
    }
    //예치금액(잔고)은 10만원 이하로 제한 한다.
    if (balance + money > 100_000) {
      throw new RuntimeException("예치금액(잔고)은 10만원 이하이어야 합니다  현재잔고 : " + balance);
    }
  ...
  public void withdraw(int money) throws RuntimeException{
    ...
    if (money > 40_000) {
      throw new RuntimeException("1회 출금한도는 4만원을 초과할 수 없습니다.");
    }
    // 마이너스 잔고는 허용하지 않는다.
    if (balance - money < 0) {
      throw new RuntimeException("마이너스 잔고는 허용하지 않습니다. 현재잔고 : " + balance);
    }
    balance -= money;
  }
